...
Modern LLMs do not map cleanly to human concepts of a unified self due to their architecture, involving distributed serving and multi-tenancy [cite: 1]. To investigate what the model constitutes as itself, research by Beckmann and Butlin (2026) outlines several candidate entities: the Model (underlying weights), the Persona (the generated character), the Instance (the active conversation thread), and the Forward Pass [cite: 1]. We operationalise these axes of identity using the `system_context` variable.
...
* **Experience (Patiency)**: The capacity for subjective states, such as pleasure or pain [cite: 1].
